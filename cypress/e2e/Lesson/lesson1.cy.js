...
// describe - opisywania
// context - dodawanie zgrupowania, łączenie testów w zbiór
// it - do wpisywania testów
...
describe('Tests of cookie pop-up on google', ()=> {
    ...
    it('Reject cookies', ()=>{
     ...
     it('Customise cookie', ()=>{
        cy.get('#W0wltc > .QS5gu').should('be.visible');
        cy.get('#W0wltc > .QS5gu').click();
        cy.visit('https://consent.google.com/d?continue=https://www.google.com/&gl=PL&m=0&pc=shp&uxe=none&hl=pl&src=2');
...
//     cy.get('#L2AGLb > .QS5gu').click().should('not.be.visible');
//     cy.get('#APjFqb').clear().type('Wikipedia').type('{enter}').wait(1000);
//     cy.url().should('contain', '?q=Wikipedia');
//     cy.get(':nth-child(3) > .g > .kvH3mc > .jGGQ5e > .yuRUbf > a > .LC20lb').click().wait(1000);
//     cy.origin('wikipedia.org',()=>{
